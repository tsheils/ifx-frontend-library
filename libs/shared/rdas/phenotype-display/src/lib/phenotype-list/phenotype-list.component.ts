...
import { GeneAssociation, PhenotypeAssociation } from "@ncats-frontend-library/models/rdas";
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, PhenotypeListCardComponent, MatPaginatorModule, MatButtonModule,
    MatIconModule, MatSortModule, MatCardModule, MatTableModule, SharedUtilsDataNotFoundComponent,
...
export class PhenotypeListComponent implements AfterViewInit, OnChanges {
  ...
  downloadData() {
    if(this.phenotypes) {
      ...
      this._downloadFile(this._toTSV(this.phenotypes), 'rdas-phenotypes-download.tsv')
    }
  }

  _toTSV(data: PhenotypeAssociation[]): string {
// grab the column headings (separated by tabs)
    const headings: string = ["term","frequency","validationStatus", "evidence","references"].join("\t");
