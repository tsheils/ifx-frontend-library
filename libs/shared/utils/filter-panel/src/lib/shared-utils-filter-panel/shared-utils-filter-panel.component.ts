...
export class SharedUtilsFilterPanelComponent implements OnInit, OnChanges {
  ...
  @ViewChild(CdkVirtualScrollViewport, {read: CdkVirtualScrollViewport, static: true}) cdkViewport!: CdkVirtualScrollViewport;
  ...
  ngOnInit() {
    ...
    this.searchCtl.valueChanges
      .pipe(
        ...
        map(term => {
            this.filterChange.next({ label: this.filter.label, term: term ? term : " " })
            this.loading = true;
        })
      ).subscribe()
...
ngOnChanges() {
    this.filterSelection.select(...this.filter.values.filter(val => val.selected).map(val=> val.term))
}
 ...
 scrollDetected(){
  ...
  loadMore() {
    this.loading = true;
    this.filterChange.next({ label: this.filter.label, page: this.filter.page })
}
  clearSearch() {
