  ...
  makeChart() {
    ...
    const seriesIndex: InternMap = index(this.data.values, (d:Filter) => d.term, (d:Filter) => d.label) as InternMap;
     this.series =  <unknown>stack()
        ...
        (seriesIndex as Iterable<{ [key: string]: number; }>) as Stack<never, { [key: string]: number }, string>;  // group by stack then series key
    ...
    this.bars = this.svg.append("g")
     ...
     .on("mouseover", (event: Event, d: ChartPoint) => this.pointerMoved(event, d))
...
*/
  ...
  pointerMoved(event: Event, d: ChartPoint){
    ...
    Array.from((d.data[1]).entries()).forEach((value:[string, {[key: string]: unknown}] ) => {
      dataString.push(value[1]['label'] + ': ' + value[1]['count'] as string)
    })
    ...
    const path: Selection<BaseType | SVGPathElement, undefined, null, undefined> = this.tooltip.selectAll("path")
    ...
    const text: Selection<BaseType | SVGTextElement, undefined, null, undefined> = this.tooltip
      .selectAll("text")
      // eslint-disable-next-line no-sparse-arrays
      .data([,])
      .join("text")
  ...
  // Wraps the text with a callout path of the correct size, as measured in the page.
   size(text: Selection<BaseType | SVGTextElement, undefined, null, undefined>, path: Selection<BaseType | SVGPathElement, undefined, null, undefined>) {
     if(text.node()) {
