...
export class SharedUtilsPieChartComponent extends GenericChartComponent implements OnInit, OnChanges {
  ...
  makeChart() {
   ...
   .append("g")
    ...
    this.svgExport = <SVGElement>select(this.chartElement.nativeElement).select('svg').node()
  }
  ...
  addTooltip(d: Filter): void {
    ...
    this.svg.append('text')
      .attr('class', 'toolCircle')
      .attr("style", "text-anchor: middle; font-family: Roboto, serif; font-color: white")
      .attr('dy', -15) // hard-coded. can adjust this to adjust text vertical alignment in tooltip*/
      .html(d.term); // add text to the circle.
    this.svg.append('text')
      .attr('class', 'toolCircle value')
      .attr("style", "text-anchor: middle; font-family: Roboto, serif; font-size: 2.4em")
