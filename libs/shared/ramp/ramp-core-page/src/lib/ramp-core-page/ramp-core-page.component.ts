...
export class RampCorePageComponent {
  ...
  visualizationMap: Map<string, {type: string,  data: {tooBig?: boolean, image?: SafeHtml, values?: unknown}}> = new Map<string, {type: string,  data: {tooBig?: boolean, image?: SafeHtml, values?: unknown}}>();
  dataMap: Map<string, {data: { [key: string]: DataProperty }[], fields:  DataProperty[]}> = new Map<string, {data: { [key: string]: DataProperty }[], fields:  DataProperty[] }>();
...
/*  _getSupportedIds() {
  ...
  protected _mapData<T extends RampDataGeneric>(data: T[]): { [p: string]: DataProperty }[] {
    const dataAsDataProperty: { [key: string]: DataProperty }[] = data.map((obj: T) => {
