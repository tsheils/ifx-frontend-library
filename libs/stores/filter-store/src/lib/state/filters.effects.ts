...
const filterMap: Map<string, {query: DocumentNode, parameters: { [key: string]: unknown}}> = new Map<string, {query: DocumentNode, parameters: { [key: string]: unknown}} >([
  [
    'phenotypes',
    {
        ...
        return combineLatest(queries).pipe(
          map((res: unknown) => {
            ...
            if (data) {
              ...
              return FetchFiltersActions.fetchFiltersSuccess({ filters: filters });
            } else {
      ...
      (
        ...
        return actions$.pipe(
          ...
          mergeMap(([action, currentFilters]) => {
            ...
            const currentFiltersMap: Map<string, FilterCategory> = new Map<string,
              FilterCategory>();
            ...
            return combineLatest(queries).pipe(
              map((res: { [key: string]: ApolloQueryResult<unknown> }) => {
                if (res) {
                  ...
                  const filter = parseFilterResponse(res as FilterResponse, tempFilter);
                  return FetchFiltersActions.fetchFiltersSuccess({ filters: filter });
                } else
                  return FetchFiltersActions.fetchFiltersFailure({
                    error: 'No Disease found',
                  });
