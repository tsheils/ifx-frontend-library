...
export const loadData = createEffect(
  (actions$ = inject(Actions), ifxToolsService = inject(IfxToolsService)) => {
    return actions$.pipe(
      ...
      exhaustMap(() => {
        return ifxToolsService.fetchData()
          .pipe(
          map(
            (data: unknown[]) => {
              const retData: Tool[] = data.map((obj) => new Tool(obj as Partial<Tool>))
              const tempAudience:string[] = [];
              retData.forEach(tool => {
                if(tool.audience && tool.audience.length) {
                  tempAudience.push(...tool.audience.split('|').map(val => val.trim()))
                }
              });
              ...
              retData.forEach(tool => {
                if(tool.category && tool.category.length) {
                  tempCategory.push(...tool.category.split('|').map(val => val.trim()))
                }
              });
              const retCategory: string[] = Array.from(new Set(tempCategory))
              return LoadToolsActions.loadToolsSuccess({ tools: retData, audienceList: retAudience, categoryList: retCategory });
            },
