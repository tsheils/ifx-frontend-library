export class OpenApiPath {
  ...
  constructor(obj: {[key: string]: any}) {
    ...
    if(obj['x-allowedTypes']) {
      this.allowedTypes = <{metabolites: string[], 'genes/proteins': string[] }>obj['x-allowedTypes']
    }

    if(obj['requestBody'] && obj['requestBody']['content']) {
      Object.entries(<{ [key: string]: unknown }[]>obj['requestBody']['content']['application/json']['schema']['properties']).forEach(([key, value]) => {
        if(key !='fishers_results' && !<boolean>value['hidden']) {
          this.properties.push({ ...value, field: key, parent: this.parent || ''});
        }
      })

      Object.entries(obj['requestBody']['content']['application/json']['schema']['example']).forEach(([key, value]) => {
        if(key !='fishers_results') {
          this.example.push({ value: value, field: key, parent: this.parent || ''});
        } else {
              ...
              switch(key) {
               ...
               //   this.example.push({ value: valArr[0], field: key, parent: this.parent || ''});
                ...
                default: {
                  const val = value as {[key: string]: unknown}
                  if(!val['hidden']) {
                    this.example.push({ value: val[key], field: key, parent: this.parent || '' });
                  }
                  break;
                }
              }
