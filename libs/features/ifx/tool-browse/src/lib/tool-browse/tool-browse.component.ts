...
export class ToolBrowseComponent implements OnInit {
  ...
  allTools: Signal<Tool[]> = this.store.selectSignal(
    IFXToolsSelectors.selectAllIfxTools,
  );
  ...
  ngOnInit() {
    this.searchFormCtl.valueChanges
      ...
      .subscribe((term) => {
        if (term && term.length) {
          this.typeAheadValues = this.allTools()?.filter((tool: Tool) =>
            tool?.toolName
              ?.toLocaleLowerCase()
  ...
  searchString() {
    this.displayedTools.set(
      this.allTools()?.filter((tool: Tool) =>
        tool?.toolName
          ?.toLocaleLowerCase()
          .includes(this.searchFormCtl.value.toLocaleLowerCase()),
      ),
    );
  ...
  selectTool(event: MatAutocompleteSelectedEvent) {
    ...
    this.displayedTools.set(
      this.allTools()?.filter((tool: Tool) =>
        tool?.toolName?.includes(event.option.value),
      ),
    );
  }
}
