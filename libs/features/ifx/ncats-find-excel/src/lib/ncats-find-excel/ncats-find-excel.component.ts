...
export class NcatsFindExcelComponent {
  ...
  imgSrcBase = computed(() =>'./assets/images/' + this.tool()?.toolName.toLowerCase().replace(/ /g, '-'));
  fileSrc = computed(() => `./assets/files/${this.tool()?.toolName.toLowerCase().replace(/ /g, '-')}/SetupNCATSFind_private.zip`);
  images = computed(()=> {
    return [
