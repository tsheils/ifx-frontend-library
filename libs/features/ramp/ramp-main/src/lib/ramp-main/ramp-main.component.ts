...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RampPageComponent, CdkScrollable, MatSidenav, MatSidenavContainer, MatSidenavContent, RouterLink],
...
export class RampMainComponent {
  ...
  inputFields = computed(()=> {
    const questionMap: Map<string, QuestionBase<string>[]> = new Map<string, QuestionBase<string>[]>();
    const inputMap: Map<string, {properties: {[key:string]: unknown }[], examples: {[key:string]: unknown }[] }> = new Map<string, {properties: {[key:string]: unknown }[], examples: {[key:string]: unknown }[] }>
    ...
    this.paths()?.forEach((path: OpenApiPath) => {
      path.properties.forEach((prop: {[key: string]: unknown}) => {
        if(inputMap.has(<string>prop['field'])) {
         const propObj: {properties: {[key:string]: unknown }[], examples: {[key:string]: unknown }[] } = inputMap.get(<string>prop['field']) as {properties: {[key:string]: unknown }[], examples: {[key:string]: unknown }[] };
          const x = propObj.properties.find((item: {[key:string]: unknown }) => item['field'] === prop['field']);
          if(!x) {
            propObj.properties.push({ ...prop });
          }
         inputMap.set(<string>prop['field'], propObj)
        } else {
          const propObj: {properties: {[key:string]: unknown }[], examples: {[key:string]: unknown }[] } = {properties: [prop], examples: [] };
          inputMap.set(<string>prop['field'], propObj)
        }
      })
      path.example.forEach(exam => {
        if(inputMap.has(<string>exam['field'])) {
         const propObj: {properties: {[key:string]: unknown }[], examples: {[key:string]: unknown }[] } = inputMap.get(exam['field']) as {properties: {[key:string]: unknown }[], examples: {[key:string]: unknown }[] };
          const x = propObj.examples.find((item: {[key:string]: unknown }) => item['field'] === exam['field']);
          if(!x) {
            propObj.examples.push({ ...exam });
          }
         inputMap.set(<string>exam['field'], propObj)
        } else {
          const propObj: {properties: {[key:string]: unknown }[], examples: {[key:string]: unknown }[] } = {examples: [exam], properties: [] };
          inputMap.set(<string>exam['field'], propObj)
        }
      })
    })
    ...
    Array.from(inputMap.entries()).forEach(([key, value]) => {
      ...
      if(questionMap.has(<string>prop['parent'])) {
        const form: QuestionBase<string>[] = questionMap.get(<string>prop['parent']) as QuestionBase<string>[]
        form.push(question)
  ...
  _mapPathToQuestion(key: string, prop: {[key: string]: unknown}, example?: {[key: string]: unknown}):QuestionBase<string> {
